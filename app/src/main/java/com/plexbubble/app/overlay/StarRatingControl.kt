 ...
 * [rating] is in the 0f..5f range; [onRatingChange] reports the same range (multiply by 2 for Plex's 0-10 scale).
 */
@Composable
    ...
    Row(
        modifier = modifier.pointerInput(Unit) {
            detectTapGestures { offset ->
                ...
                onRatingChange((starIndex + half).coerceIn(0f, starCount.toFloat()))
            }
        }.pointerInput(Unit) {
...
fun RatingValueLabel(rating: Float, modifier: Modifier = Modifier) {
    ...
    Text(text = "$rounded / 5", modifier = modifier)
}
